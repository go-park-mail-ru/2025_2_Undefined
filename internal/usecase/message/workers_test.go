...
func TestMessageUsecase_WorkerDistribute(t *testing.T) {
	...
	userChannels := make(map[uuid.UUID]chan dto.WebSocketMessageDTO)
	for i := 0; i < 3; i++ {
		userId := uuid.New()
		ch := make(chan dto.WebSocketMessageDTO, 1)
	...
	testChatID := uuid.New()
	uc.distributeChannel <- dto.WebSocketMessageDTO{
		Type:   dto.WebSocketMessageTypeNewChatMessage,
		ChatID: testChatID,
		Value:  dto.MessageDTO{},
	}
